...
let blockedUsers = [];
const NUMBER_OF_THREADS = 100; // Number of concurrent spam threads
...
// Function to generate random IP for "X-Forwarded-For" header
function generateRandomIp() {
    return `192.168.${Math.floor(Math.random() * 256)}.${Math.floor(Math.random() * 256)}`;
}


// Async function to send a single spam message
const sendSingleMessage = async (username, message, chatId, sessionId) => {
    let counter = 0;
    ...
    try {
        ...
        const url = "https://ngl.link/api/submit"; // Replace with your target URL
        const headers = {
...
const sendMessage = async (username, message, chatId, sessionId, progressMessageId) => {
    ...
    while(userSpamSessions[chatId]?.[sessionId - 1]?.isActive) {
        ...
        {
            promises.push(sendSingleMessage(username, message, chatId, sessionId))
        }

        const results = await Promise.all(promises);

        const countSuccess = results.filter(Boolean).length;
